...
class SubmarinePingApp {
  ...
  async init() {
    ...
    initWelcomeOverlay((consented) => {
      if (consented) {
        ...
        document.querySelector('#app').innerHTML = '<p>Audio permission is required for SubmarinePing to work.</p>';
      }
    });
  }
  ...
  setupEventListeners() {
    ...
    window.addEventListener('micPermissionDenied', () => {
      alert('Microphone permission is required for SubmarinePing to detect other submarines.');
    });
  }
...
const app = new SubmarinePingApp();
app.init();
