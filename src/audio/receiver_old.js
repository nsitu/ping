// Audio receiver for detecting dual-tone submarine pings
// Detects two simultaneous frequencies: submarine type (500-600Hz) + hue (650-800Hz)

import { frequencyToHue, getSubmarineFromFrequency, getFrequencyRanges } from '../core/mapping.js';
...
// Detection thresholds for dual-tone detection
const DETECTION_THRESHOLDS = {
    PEAK_SEARCH: -50,      // dB - minimum to consider as potential peak
    SIGNAL_STRENGTH: -40,  // dB - minimum for valid tone detection
    NOISE_FLOOR: -65       // dB - estimated noise floor
};

// Get frequency ranges from mapping
const FREQ_RANGES = getFrequencyRanges();export async function startListening() {
...
function onAudioFrame(frequencyData) {
    ...
    if (now - lastSelfPingTime < 1500) { // Shorter cooldown for dual-tone
        return;
    }
    ...
    // Look for dual-tone pattern: one peak in submarine range (500-600) and one in hue range (650-800)
    const submarinePeak = peaks.find(p => 
        p.frequency >= FREQ_RANGES.submarine[0] && 
        p.frequency <= FREQ_RANGES.submarine[1] &&
        p.magnitude > DETECTION_THRESHOLDS.SIGNAL_STRENGTH
    );

    const huePeak = peaks.find(p => 
        p.frequency >= FREQ_RANGES.hue[0] && 
        p.frequency <= FREQ_RANGES.hue[1] &&
        p.magnitude > DETECTION_THRESHOLDS.SIGNAL_STRENGTH
    );

    // If we have both tones, process the dual-tone ping
    if (submarinePeak && huePeak) {
        processDualTonePing(submarinePeak, huePeak, now);
    }
}

function processDualTonePing(submarinePeak, huePeak, timestamp) {
    // Decode submarine type from lower frequency (500-600 Hz)
    const modelId = getSubmarineFromFrequency(submarinePeak.frequency);
    if (!modelId) {
        console.log(`Unknown submarine frequency: ${submarinePeak.frequency.toFixed(1)} Hz`);
        return;
    }

    // Decode hue from higher frequency (650-800 Hz)
    const hue = frequencyToHue(huePeak.frequency);

    // Calculate signal strength (use stronger of the two tones)
    const strength = Math.max(submarinePeak.magnitude, huePeak.magnitude);

    console.log(`Dual-tone ping detected: ${modelId} submarine @ ${submarinePeak.frequency} Hz + hue ${hue} @ ${huePeak.frequency.toFixed(1)} Hz (${strength.toFixed(1)} dB)`);
    ...
    console.log(`${modelId} submarine detected via dual-tone: hue ${hue}, strength ${strength.toFixed(1)} dB`);
